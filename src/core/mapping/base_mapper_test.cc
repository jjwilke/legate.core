...
class TestMapper : public mapping::BaseMapper {
 public:
  TestMapper(Legion::Runtime* rt, Legion::Machine m, const LibraryContext& ctx, const BaseMapperConfig& config) :
...
class TestMapperManager {
 public:
  TestMapperManager(TestMapper* mapper) :
      mapper_(mapper),
      manager_(Legion::Runtime::get_runtime()->runtime, mapper, /*mapper_id=*/0, GetTestProc(), /*reentrant=*/true)
  {
  }
...
class MapTaskContext {
  public:
    MapTaskContext(TestMapperManager* manager, std::vector<TestStore> stores) :
      task_(MakeTestTask(kTASK_ID, stores)),
      manager_(manager)
    {
      info_ = manager_->manager()->begin_mapper_call(Legion::Internal::MAP_TASK_CALL, task_.task.get(), continuation_precondition_);
      manager->mapper()->map_task(info_, *task_.task, input, output);
    }

    ~MapTaskContext(){
      manager_->manager()->finish_mapper_call(info_);
    }
...
TEST_F(BaseMapperTest, BasicReuseTest){
  ...
  auto manager = GetTestMapperManager({.default_contiguous = false, .single_store_per_mapping = false});
...

  {
    ...
    for (auto& instance_vector : ctx.output.chosen_instances){
      for (auto& instance : instance_vector){
        EXPECT_EQ(instance.get_instance_id(), instance_id);
      }
    }
  }
...
TEST_F(BaseMapperTest, ContiguousFieldsMatch){
  ...
  auto manager = GetTestMapperManager({.default_contiguous = true, .single_store_per_mapping = false});
...

  {
    ...
    for (auto& instance_vector : ctx.output.chosen_instances){
      for (auto& instance : instance_vector){
        EXPECT_NE(instance.get_instance_id(), instance_id);
      }
    }
  }
}
...
TEST_F(BaseMapperTest, SingleStorePerMappingReuse){
  ...
  auto manager = GetTestMapperManager({.default_contiguous = true, .single_store_per_mapping = true});
  ...
  std::vector<unsigned long> instance_ids;
  {
    ...
    for (auto& instance_vec : ctx.output.chosen_instances){
      ...
      for (auto& instance : instance_vec){
        instance_ids.push_back(instance.get_instance_id());
      }
    }
  }

  {
    ...
    for (auto& instance_vec : ctx.output.chosen_instances){
      ...
      ASSERT_EQ(instance_vec.size(), 3);
      for (auto& instance : instance_vec){
        test_ids.push_back(instance.get_instance_id());
      }
    }
